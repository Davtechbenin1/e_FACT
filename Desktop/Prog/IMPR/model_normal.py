...
class Model_norm:
	def __init__(self,mother):
		self.mother = mother
		self.fact_link = os.path.abspath("./static/Facture_model1.pdf")
	...
	def Add_table(self):
		...
		self.elements.append(Spacer(1,185))
	...
	def add_pied(self,qte_dict):
		...
		if self.taxes:
			liste = [self.get_paragraph("Taxes")[0],"","",self.get_paragraph(self.format_val(self.taxes))[0]]
			th_liste.append(liste)
			
		if autre_mont:
			liste = [[self.get_paragraph(k)[0],"","",self.get_paragraph(self.format_val(v))[0]] for k,v in 
				autre_mont.items()]
			th_liste.extend(liste)
		th_liste.append([self.get_paragraph("Montant Totale")[0],'',
	...
	def Gestion_parti1(self,canvas):
		...
		typ_p,siz = self.get_paragraph(self.typ,
			'Inter',font_size = 25)
		typ_p.drawOn(canvas, self.marge_left+150, curent_y-siz[1])
		infos = self.cmd_dic.get('Information_normalisation')
		curent_y -= 30
	
		if infos:
			norm_text = f"""
			Date : {infos.get("date_time")}<br/>
			Code eMECEF :<br/> {infos.get('code_me_ce_fdgi')}<br/>
			Numéro Compteur : {infos.get("counters")}<br/>
			Nim : {infos.get('nim')}<br/>
			"""
			norm_p, norm_size = self.get_paragraph(norm_text, font_size=8,
				leading = 13)
			norm_p.drawOn(canvas, self.marge_left+150+75, curent_y - (norm_size[1] + 10))
			#curent_y -= norm_size[1] + 10

			num = self.cmd_dic.get('N°')
			data = infos['qr_code']
			img = self.mother.sc.QR_code(data,num)
			if img:
				canvas.drawImage(
					img,
					x=self.marge_left+150-6,
					y=curent_y - 80,
					width=90,
					height=80,
	...
	def Gestion_parti2(self,canvas):
		...
		plan_date = [DAT_T.datetime.strptime(i,"%d-%m-%Y") for i in self.cmd_dic.get('plan de paiements')]
		if plan_date:
			date_ech = max(plan_date).strftime("%d-%m-%Y")
		else:
			date_ech = str()
		...
		for k,v in dic.items():
			st+= f"{k} : {v}<br/>"
		p,s = self.get_paragraph(st,width = 127,
			text_color = "gray")
